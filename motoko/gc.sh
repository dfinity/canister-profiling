...
let default = wasm_profiling("motoko/default.wasm", record { trace_only_funcs = vec{"schedule_copying_gc"}; start_page = 16; page_limit = 127 });
let copying = wasm_profiling("motoko/copying.wasm", record { trace_only_funcs = vec {"copying_gc"}; start_page = 16; page_limit = 127 });
let compacting = wasm_profiling("motoko/compacting.wasm", record { trace_only_funcs = vec{"compacting_gc"}; start_page = 16; page_limit = 127 });
let generational = wasm_profiling("motoko/generational.wasm", record { trace_only_funcs = vec{"generational_gc"}; start_page = 16; page_limit = 127 });
let incremental = wasm_profiling("motoko/incremental.wasm", record { trace_only_funcs = vec{"incremental_gc"}; start_page = 16; page_limit = 127 });
