...
let motoko = wasm_profiling("motoko/.dfx/local/canisters/dip721_nft/dip721_nft.wasm");
...
flamegraph(NFT, "NFT.mintDip721", "nft_mint");

// transfer tokens
let _ = call NFT.transferFromDip721(bob, alice, 0);
let svg = stringify(title, "_nft_transfer.svg");
output(file, stringify("[", __cost__, "](", svg, ")|\n"));
flamegraph(NFT, "NFT.transferFromDip721", "nft_transfer");
};
